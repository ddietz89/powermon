...
channels = db.query("SELECT id, channel_num FROM channel WHERE device_id = '4'")
...
while(True):
    ...
    for packet_channel in packet.channels.keys():
	    ...
	    if channel['channel_num'] == packet_channel:
	        db.execute("INSERT INTO channel_packet (channel_id, voltage, seconds, wattsec, datetime) VALUES ('" + db.escape(str(channel['id'])) + "','" + db.escape(str(packet.voltage*10)) + "','" + db.escape(str(packet.seconds)) + "','" + db.escape(str(packet.channels[packet_channel].abs_wattsecond)) + "',NOW())")
